...
const PostPage = ({ params }: PostPageProps) => {
  ...
  // Get current Convex user only if authenticated
  ...
  // Get like status for current user (only if authenticated)
  // Note: hasUserLiked should determine the user from auth context, not pass userId
  ...
  // Mutations
  const { mutate: toggleLikeMutate, isLoading: isTogglingLike } = 
    useConvexMutation(api.likes.toggleLike);

  const { mutate: addCommentMutate, isLoading: isSubmittingComment } =
    useConvexMutation(api.comments.addComment);

  const { mutate: deleteCommentMutate, isLoading: isDeletingComment } = 
    useConvexMutation(api.comments.deleteComment);

  const { mutate: incrementViewMutate } = 
    useConvexMutation(api.public.incrementViewCount);

  // Track view when post loads - only once per page load
  const hasTrackedView = useRef(false);

  useEffect(() => {
    if (post && !postLoading && !hasTrackedView.current) {
      incrementViewMutate({ postId: postId as Id<"posts"> })
        .then(() => {
  ...
  }, [post, postLoading, postId, incrementViewMutate]);
  ...
  const handleLikeToggle = async () => {
    ...
    try {
      await toggleLikeMutate({ postId: postId as Id<"posts"> });
    } catch (error) {
