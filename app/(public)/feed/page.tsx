...
export default function FeedPage() {
  ...
  // Mutations
  const { mutate: toggleFollowMutate, isLoading: isTogglingFollow } = 
  ...
  const handleFollowToggle = async (userId: Id<"users">) => {
    ...
    try {
      const result = await toggleFollowMutate({ followingId: userId });
      if (result?.following) {
                    ...
                    {suggestedUsers.map((user: any) => (
                      <div key={user._id} className="space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <Button
                            onClick={() => handleFollowToggle(user._id as Id<"users">)}
                            variant="outline"
                            size="sm"
                        ...
                        <div className="text-xs text-slate-500 pl-13">
                          {user.followerCount || 0} followers • {user.postCount || 0}{" "}
                          posts
                        </div>
                        {user.recentPosts && user.recentPosts.length > 0 && (
                          <div className="text-xs text-slate-400 pl-13">
                            ...
                            {user.recentPosts[0].title.length > 30 ? "..." : ""}
                            "
                          </div>
                        )}
                      </div>
