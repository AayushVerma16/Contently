...
type Transformation = 
  | { width?: number; height?: number; focus?: string }
  | { effect: string }
  | {
      overlayText: string;
...
interface ImageUploadModalProps {
  isOpen: boolean;
  onClose: () => void;
  onImageSelect: (image: { url: string; originalUrl?: string; fileId?: string; name?: string; width?: number; height?: number }) => void;
  title?: string;
}
...
export default function ImageUploadModal({
  ...
  const applyTransformations = async () => {
    ...
    try {
      ...
      // Text overlay
      if (watchedValues.textOverlay?.trim() && watchedValues.textColor) {
        transformationChain.push({
          ...
          gravity: watchedValues.textPosition || "center",
